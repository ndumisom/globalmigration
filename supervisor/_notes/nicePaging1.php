	...
	public function createPaging($link){
		...
		if($this->page>1){
			...
			$paging.='<li><a href="'.$link.$this->separator.'page='.($this->page-1).'" title="Previous page">&lt;</a></li>';
                         echo '<script type = "text/javascript"> document.getElementById("reload_now").value="no"; </script>';
                      
                        
		}
		
		if($start>$this->maxPages){
			$paging.='<li><a href="'.$link.$this->separator.'page='.($start-1).'" title="Page '.($start-1).'" onclick="resetAll()">...</a></li>';
                       // echo '<script type = "text/javascript"> document.getElementById("reload_now").value="no"; </script>';
		}
		...
		for($i=$start;$i<=$end;$i++){
			if($this->page==$i){
				...
				$paging.='<li><a href="'.$link.$this->separator.'page='.$i.'" title="Page '.$i.'" onclick="resetAll()">'.$i.'</a></li>';
		...
		if($end<$this->totalPages){
			$paging.='<li><a href="'.$link.$this->separator.'page='.($end+1).'" title="Page '.($end+1).'" onclick="resetAll()">...</a></li>';
                        // echo '<script type = "text/javascript"> document.getElementById("reload_now").value="no"; </script>';
		}
		
		if($this->page<$this->totalPages){
			$paging.='<li><a href="'.$link.$this->separator.'page='.($this->page+1).'" title="Next page" onclick="resetAll()">&gt;</a></li>';
			$paging.='<li><a href="'.$link.$this->separator.'page='.$this->totalPages.'" title="Last page" onclick="resetAll()">&gt;&gt;</a></li>';
                        //echo '<script type = "text/javascript"> document.getElementById("reload_now").value="no"; </script>';
		}
