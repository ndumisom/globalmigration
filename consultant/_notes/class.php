...
//Get quote id by quote number
    function get_quote_id($quote_no) {

        $user = '';

        if ($row = mysql_fetch_array(mysql_query(" select * from quotes where quote_no = '$quote_no' "))) {
            $user = $row['quote_id'];
        }
...
//insert quote discription
    ...
    function multiple_insert($descrption) {
        ...
        for ($i = 0; $i < count($description); $i++) {
            if (!empty($description[$i])) {
                ...
                $call->insert_quote_des($quote_no, $description[$i], $professional_fees[$i], $saqa_dha_fee[$i], $line_total[$i]);
            }
        }
    }
